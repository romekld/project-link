...
- Vercel CI/CD: connected to `romekld/project-link` on GitHub; auto-deploys `frontend/` on push to `main`. Production URL: `https://project-link-cho2.vercel.app`.
- `vercel.json` committed — documents root directory, build command, and output directory.
- Supabase project: not yet created.
- DigitalOcean App Platform: not yet provisioned.

### CI/CD (TG8 — complete)
- `.github/workflows/frontend-ci.yml` — runs `tsc --noEmit`, ESLint, and Vitest unit tests on every push/PR touching `frontend/`.
- `.github/workflows/backend-ci.yml` — runs `pytest` via `uv` on every push/PR touching `backend/`.
- `.github/workflows/playwright.yml` — runs Playwright E2E tests; PRs run against the local Vite dev server; pushes to `main` run against the Vercel production URL.
- Vitest `^3.1.1` added to `frontend/package.json` devDependencies with `"test": "vitest run"` script.
- `vite.config.ts` updated with `test` block (`jsdom` environment, `v8` coverage provider).

### E2E tests
- Playwright project initialized under `e2e/`.
- `playwright.config.ts` updated: `baseURL` resolved from `PLAYWRIGHT_BASE_URL` env var (falls back to `localhost:5173`); `webServer` block added for local dev; mobile viewport projects added.
- No business-logic tests written yet.
...
### CI/CD
- [x] Connect Vercel to the frontend repo; set `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY`. *(Vercel connected; env vars to be added once Supabase project is created)*
