...
|:-----------|:----------|:----------|:------|
| `user_id` | TEXT | System-generated | Human-readable format: `role-yyyy-####`. |
| `middle_name` | TEXT | Optional | Normalized middle name. |
| `name_suffix` | TEXT | Optional | Example: `Jr.`, `Sr.`, `III`. |
| `mobile_number` | TEXT | Optional | Must match `09123456789` format or can be a phone input. Recommended for field roles. |
...
- `phn` `phis_coordinator`, `city_health_officer`, and `system_admin` remain city-wide.
- `bhw` and `rhm` require `health_station_id`.
