...
| **City Health Officer** | `city_health_officer` | Web + Mobile | Read-only | All 32 BHS | Monitors GIS/ML; signs final M1/M2. |
| **PHIS Coordinator** | `phis_coordinator` | Web only | Read-only | All 32 BHS | Auditor: Performs final DQC; manages exports. |
| **Disease Surv. Officer** | `dso` | Web only | PIDSR CRUD | All 32 BHS | Validates cases; monitors real-time alerts. |
| **Public Health Nurse** | `nurse_phn` | Web only | Full CRUD | All 32 BHS | Aggregator: Reviews STs to generate MCT. |
| **Rural Health Midwife** | `midwife_rhm` | Web + Mobile | Full CRUD | Own BHS Only | Record Keeper: Manages ITR/TCL; creates ST. |
...
| **Frontend Web** | React + TypeScript | Type-safety for FHSIS; Tailwind for responsive design. |
...
- **Phase 1 (Setup):** Initialize Supabase RLS, Vercel CI/CD, and Tailwind Design System. Initialize the infrastructure setup.
- **Phase 2 (ITR/TCL):** Deploy BHW field entry and Midwife TCL management.
- **Phase 3 (ST/MCT):** Build the "Auto-Tally" engine and Nurse’s Consolidation Dashboard.
- **Phase 4 (Intelligence):** Integrate MapLibre GIS mapping and Celery-based ML forecasting.
